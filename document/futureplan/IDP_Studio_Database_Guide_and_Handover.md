...
> **Database File Location:** `automation_engine/modules/idp_studio/idp_studio.db`  
...
| :--- | :--- | :--- |
| [idp_studio/db.py](file:///Users/apple/Desktop/FLA/automation_engine/modules/idp_studio/db.py) | Configured connection to `automation_engine/modules/idp_studio/idp_studio.db` and added `init_db()`. | Places `idp_studio.db` directly inside the `idp_studio/` folder and enables automated schema creation. |
...
| [api/main.py](file:///Users/apple/Desktop/FLA/automation_engine/api/main.py#L30) | Invoked `idp_studio.db.init_db()` during application startup. | Ensures any developer who runs the backend server gets `idp_studio.db` created automatically zero-touch. |

---

## 2. Developer Guide: How to Use `idp_studio.db`

### A. Where is the Database File Located?
The SQLite database file is generated directly inside the `idp_studio` directory:
* **Path:** `automation_engine/modules/idp_studio/idp_studio.db`

---

### B. How to Create a New Table (for DOM Learner or IDP Rules)
...
3. **What happens:** `init_db()` automatically detects `DomLearnerRule` and creates the `dom_learner_rules` table inside your local `idp_studio.db`!

---

### C. How to Share Your New Table with the Team
...
```
* **Why this works:** When another developer runs `git pull` and starts their app, `init_db()` automatically builds your new table in their local `idp_studio.db`!

---

### D. How to Query or Insert Data in Python Code
...
| **Resetting Your DB** | Delete `idp_studio.db` locally and restart the app (`uvicorn main:app`). | Don't copy `.db` files from another developer's computer. |
