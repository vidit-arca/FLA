...
import IdpProductionApp from './idp_studio/IdpProductionApp';
...
function App() {
  ...
  const isIdpExtract = window.location.pathname === '/idp-extract';
  ...
  if (isIdpExtract) {
    return (
      <Router>
        <Routes>
          <Route path="/idp-extract" element={<IdpProductionApp />} />
        </Routes>
      </Router>
    );
  }
  ...
  return (
    <Router>
      <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] flex font-sans text-slate-800 dark:text-slate-200">
        {/* Sidebar */}
        <div className="w-72 h-screen sticky top-0 bg-slate-100 dark:bg-[#0F1523]/90 backdrop-blur-3xl border-r border-slate-200 dark:border-white/[0.05] flex flex-col z-20 shadow-2xl shrink-0">
          ...
          <div className="p-6 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-3 mb-1">
              ...
              <div className="flex flex-col">
                <h1 className="text-lg font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">AKSHAYAM</h1>
                <p className="text-[0.6rem] text-indigo-300/80 font-semibold uppercase tracking-wider mt-1">Extraction Platform</p>
              </div>
            </div>
            <button onClick={toggleTheme} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10 transition-colors" title="Toggle Theme">
          ...
          <nav className="flex-1 px-4 space-y-1 mt-1 overflow-y-auto hide-scrollbar">
            <div className="h-px bg-slate-900/5 dark:bg-white/5 mb-4 mx-3" />
            <p className="px-3 text-[0.65rem] font-bold text-slate-500 uppercase tracking-wider mb-2">Main Menu</p>

            <NavLink
              to="/"
              end
              className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 font-medium text-sm ${isActive ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shadow-inner' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 hover:bg-slate-900/5 dark:bg-white/5 border border-transparent'}`}
            >
          ...
          <div className="p-4 mt-auto shrink-0">
            <div className="border-t border-slate-200 dark:border-white/5 pt-4 space-y-3">
              <div className="w-full flex items-center gap-3 px-3 py-2 bg-black/20 rounded-xl border border-slate-200 dark:border-white/5 cursor-pointer hover:bg-black/30 transition-colors">
                <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-slate-600 to-slate-400 flex items-center justify-center text-slate-900 dark:text-white border border-slate-500 shrink-0">
                  <User className="w-3.5 h-3.5" />
                </div>
                <div className="flex flex-col overflow-hidden">
                  <span className="text-[13px] font-semibold text-slate-900 dark:text-white truncate">Admin User</span>
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto relative">
          ...
          <div className="w-full h-full p-6 lg:p-8 relative z-10 animate-fade-in">
            <Routes>
