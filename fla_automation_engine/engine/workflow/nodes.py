...
from ..validator import ReturnValidator
from ..ocr_pipeline import OcrPipeline
from ..comparison_platform.manager import ComparisonPlatformManager
...
def node_output(state: WorkflowState) -> WorkflowState:
    ...
    # Path logic: __file__ is inside engine/workflow/nodes.py
    # engine_root is ../../ (fla_automation_engine/)
    engine_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    skeletal_path = os.path.join(engine_root, "excel", "FLA Return existing skeletal.xlsx")
...
def node_compare(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        results = manager.run_comparison("fla", state["previous_fla_file"], state["output_excel"])
        state["comparison_results"] = results
        
        success_msg = f"  -> Comparison completed. {len(results)} rules evaluated."
        print(success_msg)
