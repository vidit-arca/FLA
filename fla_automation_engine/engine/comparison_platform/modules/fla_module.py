...
class FLAComparisonModule(BaseComparisonPlatformModule):
    def __init__(self, template_path: str = "/Users/apple/Desktop/FLA/fla_automation_engine/excel/FLA_comparsion.xlsx"):
        ...
        for sheet_name, template_df in template_dfs.items():
            ...
            if field_name_col != -1:
                ...
                for idx, row in template_df.iterrows():
                    ...
                    col_map = [
                        (tmp_py_share, py_val, "PY Shares/Col1"),
                        (curr_py_amount, py_val, "PY Amount/Col2"),
                        (tmp_fy_share, fy_val, "FY Shares/Col1"),
                        (tmp_fy_amount, fy_val, "FY Amount/Col2"),
                    ]

                    for col_idx, fill_val, label in col_map:
                        if col_idx == -1 or not fill_val or idx >= len(tgt_df):
                            continue
                        curr = self._normalize_val(
                            tgt_df.iloc[idx, col_idx] if col_idx < len(tgt_df.columns) else None
                        )
                        if not curr:
                            if self._safe_write(target_ws, idx + 1, col_idx + 1, fill_val):
                                has_modifications = True
                            results.append({
                                "fieldName": f"{sheet_name} | {field_name} ({label})",
                                "mappingType": "PY_FY_COLUMN_FILL",
                                "previousValue": fill_val,
                                "currentValue": curr,
                                "finalSelectedValue": fill_val,
                                "reason": f"{label} column empty -> Filled from previous FLA"
                            })
