...
To start the backend FastAPI server, run the following command from the `automation_engine` directory:
```bash
...
python3 run_pipeline.py \
  ...
  --ocr-dir "/Users/apple/Desktop/FLA/automation_engine/output/marker/kiritlabs" \
...
`automation_engine/output/marker/{PDF_BASE_NAME}/{PDF_BASE_NAME}.json`
