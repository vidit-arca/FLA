...
xl = pd.ExcelFile("/Users/apple/Desktop/FLA/automation_engine/excel/FLA_Tool_v5_fixed.xlsx")
df = pd.read_excel(xl, sheet_name="2_FINANCIALS")

print("--- Section IV Rules (Cols C & D) ---")
for r_idx in range(160, 180):
    if r_idx < len(df):
        row = df.iloc[r_idx]
        ...
        if col_b:
            print(f"Row {r_idx+2} | B: {col_b[:40]} | C (PY): {col_c[:20]} | D (FY): {col_d[:20]}")
