...
def main():
    ...
    default_skeletal = os.path.join(engine_dir, "modules", "fla", "excel", "FLA Return existing skeletal.xlsx")
    ...
    default_config = os.path.join(engine_dir, "modules", "fla", "rules_config.json")
    ...
    for key_name, doc_path in docs.items():
        ...
        if cached["md"] or cached["json"]:
            ...
            if is_scanned:
                ...
                print(f"    [!] ACTION REQUIRED: Please run `python automation_engine/run_ocr_batch.py` first to safely process this heavy ML task.")
            else:
